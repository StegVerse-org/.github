...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--request",type=Path,required=True); ap.add_argument("--packet-out",type=Path,required=True); ap.add_argument("--frame-out",type=Path)
    ...
    print(json.dumps({"status":"PASS","packet_id":packet["packet_id"],"destination":packet["destination"],"manifest_sha256":req["bindings"]["manifest_sha256"]},sort_keys=True))
